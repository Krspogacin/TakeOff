...
function SeatchartJS(seatMap, seatTypes) { // eslint-disable-line no-unused-vars
    ...
    } else if (seatMap.rows > 26 || seatMap.cols > 26) {
        throw new Error("Invalid parameter 'seatMap' supplied to SeatchartJS. " +
            "'row' and 'cols' properties cannot be integers greater than 26.");
    ...
    */
     ...
     */
    var createRow = function createRow(rowIndex, i) {
        ...
        if (rowIndex === undefined) {
            ...
            row.appendChild(createSeat('index', rowIndex, i));
        }
    ...
    this.addRow = function () {
        ...
        var rowIndex = alphabet[seatMap.rows];
        var row = createRow(rowIndex);

        for (var j = 0; j < seatMap.cols; j += 1) {
            row.appendChild(createSeat('available', rowIndex + (j + 1), seatMap.rows + '_' + j));
        }
        ...
        if (seatMap.disabledCols) {
            for (var k = 0; k < seatMap.disabledCols.length; k += 1) {
                ...
                seatMap.disabled.push((seatMap.cols * seatMap.rows) + disabledColumn);
            }
        }
    ...
    this.createMap = function createMap(containerId) {
        ...
        seatMapContainer = createContainer();
